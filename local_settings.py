...
# **IF USING DOCKER LOCALLY, YOU MUST EITHER SET THIS TO 'BLANK' OR TO THE EXTERNAL DOMAIN/IP OF THE DOCKER MACHINE (NOT THE DOCKER CONTAINER)
DJANGO_URL = 'http://metashare.myorg.org'
...
STORAGE_PATH = '/metashare_resources'

# STATIC FILES
STATIC_ROOT =  "/var/www/metashare_static"
STATIC_URL = "%s/site_media/" $ DJANGO_URL
STATICFILES_DIRS = (
    "/META-SHARE-3.0.2/metashare/media",
)
...
ADMINS = (
     ('admin', 'metashare-admin@myorg.org'),
)
...
DATABASES = {
    'default': {
        'ENGINE': 'django.db.backends.postgresql_psycopg2', # Add 'postgresql_psycopg2', 'mysql',
                                         # 'sqlite3', 'oracle'.
        'NAME': 'metashare',                      # database name -- or path to file if
                                         # using sqlite3, e.g.
                                         # '{0}/testing.db'.format(ROOT_PATH)
        'TEST_NAME': 'metashare_test',                 # test database name -- or path to file
                                         # if using sqlite3, e.g.
                                         # '{0}/testing2.db'.format(ROOT_PATH)
                                         # mandatory when using Selenium tests
        'USER': 'metashare',                      # Not used with sqlite3.
        'PASSWORD': 'metashare',                  # Not used with sqlite3.
        'HOST': 'localhost',                      # Set to 'localhost' for localhost.
